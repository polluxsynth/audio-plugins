...
#define PARAM(PARAMNO, PG, SP, NAME, SYMBOL, MIN, MAX, DEFAULT, SETFUNC)
	...
	PARAMGROUP(PG_LFO3, "Modulation 3", "g303_mod_lfo3")
	...
	PARAMPOINTS(SP_ASGNRES, 0, " Off ", " Res ")
	PARAMPOINTS(SP_ASGNMTRG, 0, " All ", "Note On", " None ")
	PARAMPOINTS(SP_ENVRST, 0, "FreeRun", "KeyReset")
	PARAMPOINTS(SP_EXPLIN, 0, "Exp", "Lin")
	PARAMPOINTS(SP_LFOWAVE, 0, " Off ", " Saw ", " Fall ", " Tri ", " Rise ", " RSaw ", "Pul ", " Squ ", " RPul ", " S/H ")
	PARAMPOINTS(SP_LFOSYNC, 0, " Off ", "Clock", " Key ", "OneShot")
	PARAMPOINTS(SP_LFOCONTR, 0, " None ", " Modw ", "Aftert", " Vel ")
	PARAMPOINTS(SP_LFODEST, 0, "Off", "Osc1", "Osc1+2", "Osc2", "Sh1", "Sh1+2", "Sh2", "Cutoff", "Res", "Osc2Filt")
	PARAMPOINTS(SP_POLARITY, 0, " Norm ", " Inv ", " Uni ", "InvUni")
	PARAMPOINTS(SP_OSCWAVE, 0, " Off ", " Saw ", " Pul ", " Tri ")
	...
	PARAM(ASGN_MEM, PG_KEYASGN, SP_ASGNMEM, "Assign Memory", "keyassignmem", SP_MIN, SP_MAX, 1, setKeyAsgnMem)
        // Rob a playing voice if no unplaying available
        // two modes: oldest (O) and next-to-lowest (NL)
	PARAM(ASGN_ROB, PG_KEYASGN, SP_ASGNROB, "Voice Rob", "keyassignrob", SP_MIN, SP_MAX, 2, setKeyAsgnRob)
	...
	PARAM(LFO1WAVE, PG_LFO1, SP_LFOWAVE, "Wave", "lfo1wave", SP_MIN, SP_MAX, 0, setLfo1Wave)
	PARAM(LFO1AMT, PG_LFO1, SP_NONE, "Initial Amount", "lfo1amount", 0, 10, 0, setLfo1Amt)
	PARAM(LFO1DEST, PG_LFO1, SP_LFODEST, "Dest", "lfo1dest", SP_MIN, SP_MAX, 0, setLfo1Dest)
	...
	PARAM(LFO1_AMT_CTRL, PG_LFO1, SP_LFOCONTR, "Controller", "lfo1amtcont", SP_MIN, SP_MAX, 0, setLfo1Controller)
	PARAM(LFO1_POLARITY, PG_LFO1, SP_POLARITY, "Polarity", "lfo1polarity", SP_MIN, SP_MAX, 0, setLfo1Polarity)
	PARAM(LFO1SYNC, PG_LFO1, SP_LFOSYNC, "Sync", "lfo1sync", SP_MIN, SP_MAX, 0, setLfo1Sync)

	// LFO2 #1 (main: freq, wave, basic amount)
	PARAM(LFO2FREQ, PG_LFO2, SP_NONE, "Rate", "lfo2frequency", 0, 10, 6, setLfo2Frequency)
	PARAM(LFO2WAVE, PG_LFO2, SP_LFOWAVE, "Wave", "lfo2wave", SP_MIN, SP_MAX, 0, setLfo2Wave)
	PARAM(LFO2AMT, PG_LFO2, SP_NONE, "Initial Amount", "lfo2amount", 0, 10, 0, setLfo2Amt)
	PARAM(LFO2DEST, PG_LFO2, SP_LFODEST, "Dest", "lfo2dest", SP_MIN, SP_MAX, 0, setLfo2Dest)
	...
	PARAM(LFO2_AMT_CTRL, PG_LFO2, SP_LFOCONTR, "Controller", "lfo2amtcont", SP_MIN, SP_MAX, 0, setLfo2Controller)
	PARAM(LFO2_POLARITY, PG_LFO2, SP_POLARITY, "Polarity", "lfo2polarity", SP_MIN, SP_MAX, 0, setLfo2Polarity)
	PARAM(LFO2SYNC, PG_LFO2, SP_LFOSYNC, "Sync", "lfo2sync", SP_MIN, SP_MAX, 0, setLfo2Sync)

	// LFO3 #1 (main: freq, wave, basic amount)
	PARAM(LFO3FREQ, PG_LFO3, SP_NONE, "Rate", "lfo3rate", 0, 10, 6, setLfo3Frequency)
	PARAM(LFO3WAVE, PG_LFO3, SP_NONE, "Shape", "lfo3shape", 0, 10, 2.5, setLfo3Shape)
	PARAM(LFO3AMT, PG_LFO3, SP_NONE, "Initial Amount", "lfo3amount", 0, 10, 0, setLfo3Amt)
	PARAM(LFO3DEST, PG_LFO3, SP_LFODEST, "Dest", "lfo3dest", SP_MIN, SP_MAX, 0, setLfo3Dest)
	// LFO3 #2 (secondary: sync, controllers, polarity)
	PARAM(LFO3_CONTRAMT, PG_LFO3, SP_NONE, "Controller Amount", "lfo3contramt", 0, 10, 0, setLfo3ControllerAmt)
	PARAM(LFO3_AMT_CTRL, PG_LFO3, SP_LFOCONTR, "Controller", "lfo3amtcont", SP_MIN, SP_MAX, 0, setLfo3Controller)
	PARAM(LFO3_POLARITY, PG_LFO3, SP_POLARITY, "Polarity", "lfo3polarity", SP_MIN, SP_MAX, 0, setLfo3Polarity)
	PARAM(LFO3SYNC, PG_LFO3, SP_LFOSYNC, "Sync", "lfo3sync", SP_MIN, SP_MAX, 0, setLfo3Sync)
	// Osc 1
	...
	PARAM(OSC1WAVE, PG_OSC1, SP_OSCWAVE, "Wave", "osc1wave", SP_MIN, SP_MAX, 0, setOsc1Wave)
	...
	PARAM(OSC3WAVE, PG_MIXER, SP_OSC3WAVE, "Osc2Sub Wave", "osc2subwave", SP_MIN, SP_MAX, 0, setOsc2SubWave)

	// Filter configuration
	PARAM(RESPONSE, PG_FILTERCFG, SP_NONE, "Pole Count", "response", 1, 4, 4, setResponse)
	PARAM(FENV_INVERT, PG_FILTERCFG, SP_ONOFF, "Env Invert", "fenvpolarity", SP_MIN, SP_MAX, 0, setInvertFenv)
	PARAM(FENV_LINEAR, PG_FILTERCFG, SP_EXPLIN, "Env Shape", "fenvmode", SP_MIN, SP_MAX, 0, setFenvLinear)
	...
	PARAM(ENV_MODE, PG_VCA, SP_ENVMODE, "Env/VCA Mode", "envmode", SP_MIN, SP_MAX, 0, setEnvMode)
	...
	// DSP control
	PARAM(OVERSAMPLE, PG_DSP, SP_ONOFF, "Oversample", "oversample", SP_MIN, SP_MAX, 1, setOversampling)
	PARAM(ECONOMY_MODE, PG_DSP, SP_ONOFF, "Economy Mode", "economymode", SP_MIN, SP_MAX, 1, setEconomyMode)
