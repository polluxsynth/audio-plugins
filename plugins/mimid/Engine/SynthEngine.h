...
public:
	...
	{
		float porta = logsc(10 - param, 0.14, 250, 150);
	...
	void setOsc1Det(float param)
	{
		...
		ForEachVoice(osc.osc1Det = linsc(param * 10, 0, 1.0));
	}
	void setOsc2Det(float param)
	{
		ForEachVoice(osc.osc2Det = linsc(param * 10, 0, 1.0));
	}
	...
	{
		ForEachVoice(setEnvSpreadAmt(linsc(param, 0.0, 1.0)));
	}
